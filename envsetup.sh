...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^maxi_") ; then
       MAXI_BUILD=$(echo -n $1 | sed -e 's/^maxi_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $MAXI_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       MAXI_BUILD=
    fi
    export MAXI_BUILD
...
function breakfast()
{
    target=$1
    local variant=$2
    MAXI_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/maxi/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the MaxiCM model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch maxi_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the MaxiCM github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var MAXI_VERSION)
        ZIPFILE=MaxiCM-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.maxi.device | grep -q "$MAXI_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $MAXI_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.maxi.device | grep -q "$MAXI_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MAXI_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/maxi/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
